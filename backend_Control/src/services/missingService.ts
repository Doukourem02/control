...
import type { MissingRow } from '../types/control';
import { triggerStockAnomalyAlert, triggerStockLowAlert } from './notificationsTriggers';
...
export async function createMissingRecord(input: CreateMissingInput): Promise<MissingRow> {
  ...
  triggerStockAnomalyAlert(input.shopId, productDoc['name'] as string, input.quantity, currentQuantity).catch(() => {});
