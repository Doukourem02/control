...
type ProductsService = typeof import('../../src/services/productsService');
...
// null == "produit introuvable" (databases.getDocument leve), sinon le doc renvoye.
let mockProductById: unknown = { ...BASE_PRODUCT };
...
before(() => {
  ...
  // products.repository a ete fusionne dans products.service : on simule
  ...
  req.cache[appwritePath] = {
    exports: {
      DATABASE_ID: 'db-test',
      COLLECTIONS: { products: PRODUCTS, stockMovements: STOCK_MOVEMENTS, activityLogs: ACTIVITY_LOGS },
      databases: {
        getDocument: async (_db: string, collectionId: string) => {
          if (collectionId !== PRODUCTS) throw new Error('unexpected collection');
          if (!mockProductById) throw new Error('not found');
          return mockProductById;
        },
        updateDocument: async (_db: string, collectionId: string, id: string, fields: Record<string, unknown>) => {
          if (collectionId === PRODUCTS) return { ...BASE_PRODUCT, $id: id, ...fields };
          return { $id: id, ...fields };
        },
        deleteDocument: async () => undefined,
        createDocument: async (_db: string, collectionId: string, id: string, data: Record<string, unknown>) => {
          if (collectionId === PRODUCTS) return { $id: 'p-new', ...data };
          return { $id: id, ...data };
        },
  ...
  delete req.cache[req.resolve('../../src/services/productsService')];
  ({ createOrSupplyProduct, updateProduct, archiveProduct } = req('../../src/services/productsService') as ProductsService);
});
