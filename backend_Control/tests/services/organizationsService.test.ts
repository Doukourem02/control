...
type OrganizationsService = typeof import('../../src/services/organizationsService');
...
before(() => {
  // organizations.repository a ete fusionne dans organizations.service : on
  // simule directement le client Appwrite plutot qu'un module repository.
    ...
    exports: {
      ...
      databases: {
        listDocuments: async (_db: string, collectionId: string) => {
          if (collectionId !== ORGANIZATIONS) return { documents: [] };
          return { documents: mockOrganizationByOwner ? [mockOrganizationByOwner] : [] };
        },
        createDocument: async (_db: string, _collectionId: string, _id: string, data: Record<string, unknown>) => {
          createOrganizationCalls += 1;
          return { $id: 'org-new', ...data };
        },
        getDocument: async () => {
          if (!mockOrganizationByOwner) throw new Error('not found');
          return mockOrganizationByOwner;
        },
      },
    },
  } as unknown as NodeJS.Module;

  const shopsRepoPath = req.resolve('../../src/services/shopsService');
  ...
  delete req.cache[req.resolve('../../src/services/organizationsService')];
  ({ ensureOrganizationForOwner, createStoreForOwner, switchActiveStore } = req('../../src/services/organizationsService') as OrganizationsService);
});
