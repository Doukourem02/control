...
function BottomNav({
  ...
  const items = baseItems.filter((item) => role === 'seller' ? item.key !== 'report' : true);
...
export function ControlHomeScreen({ experienceRole }: { experienceRole?: ControlExperienceRole }) {
  ...
  function handleTabChange(key: NavKey) {
    if (activeRole === 'seller' && key === 'report') return;
  ...
  useEffect(() => {
    if (activeRole === 'seller' && activeMenu === 'report') {
      setActiveMenu('home');
    }
  }, [activeMenu, activeRole]);
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#FFFFFF' }}>
      ...
      <View style={{ flex: 1, alignItems: 'center', backgroundColor: '#FFFFFF' }}>
        ...
        >
          ...
          <BottomNav active={activeMenu} compact={compact} role={activeRole} onChange={handleTabChange} />
        </View>
      </View>
